...
test('should throw if missing closing bracket', () => {
	...
	result.toThrow(ErrorMissingCloseBracket);
	result.toThrowErrorMatchingInlineSnapshot(dedent`
		[ErrorMissingCloseBracket: Missing closing bracket "}}" for parameter "name!..." starting at position 7]
	`);
});
