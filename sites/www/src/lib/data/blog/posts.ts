import type { ExternalPost, Post } from '$lib/data/blog';
import { post as post_20231009_behind_the_screen_a_yes_code_blog_of_svelte_vietnam } from '$routes/(localized)/blog/(posts)/20231009-behind-the-screen-a-yes-code-blog-of-svelte-vietnam/_page/data';
import { post as post_20231012_svelte_vietnam_from_local_to_global } from '$routes/(localized)/blog/(posts)/20231012-svelte-vietnam-from-local-to-global/_page/data';
import { post as post_20231020_lets_write_a_simple_svelte_preprocessor } from '$routes/(localized)/blog/(posts)/20231020-lets-write-a-simple-svelte-preprocessor/_page/data';
import { post as post_20231110_behind_the_screen_dark_mode_with_sveltekit_tailwindcss_and_postcss } from '$routes/(localized)/blog/(posts)/20231110-behind-the-screen-dark-mode-with-sveltekit-tailwindcss-and-postcss/_page/data';
import { post as post_20231204_behind_the_screen_a_few_secrets_of_sveltevietnam_dev } from '$routes/(localized)/blog/(posts)/20231204-behind-the-screen-a-few-secrets-of-sveltevietnam-dev/_page/data';
import { post as post_20231220_behind_the_screen_progressive_splashscreen } from '$routes/(localized)/blog/(posts)/20231220-behind-the-screen-progressive-splashscreen/_page/data';
import { post as post_20240125_styling_svelte_vietnam_part_i_tailwindcss } from '$routes/(localized)/blog/(posts)/20240125-styling-svelte-vietnam-part-i-tailwindcss/_page/data';
import { post as post_20240125_styling_svelte_vietnam_part_ii_css_components } from '$routes/(localized)/blog/(posts)/20240125-styling-svelte-vietnam-part-ii-css-components/_page/data';
import { post as post_20240125_styling_svelte_vietnam_part_iii_code_discovery_portability } from '$routes/(localized)/blog/(posts)/20240125-styling-svelte-vietnam-part-iii-code-discovery-portability/_page/data';
import { post as post_20240414_introduction_to_svelte_action_runtime_superpower } from '$routes/(localized)/blog/(posts)/20240414-introduction-to-svelte-action-runtime-superpower/_page/data';
import { post as post_20240420_come_for_svelte_stay_for_the_web } from '$routes/(localized)/blog/(posts)/20240420-come-for-svelte-stay-for-the-web/_page/data';
// HYGEN MARKER - IMPORT [DO NOT REMOVE]

export const INTERNAL_POSTS: Post[] = [
	// HYGEN MARKER - POST [DO NOT REMOVE]
	post_20240420_come_for_svelte_stay_for_the_web,
	post_20240414_introduction_to_svelte_action_runtime_superpower,
	post_20240125_styling_svelte_vietnam_part_iii_code_discovery_portability,
	post_20240125_styling_svelte_vietnam_part_ii_css_components,
	post_20240125_styling_svelte_vietnam_part_i_tailwindcss,
	post_20231220_behind_the_screen_progressive_splashscreen,
	post_20231204_behind_the_screen_a_few_secrets_of_sveltevietnam_dev,
	post_20231110_behind_the_screen_dark_mode_with_sveltekit_tailwindcss_and_postcss,
	post_20231012_svelte_vietnam_from_local_to_global,
	post_20231009_behind_the_screen_a_yes_code_blog_of_svelte_vietnam,
	post_20231020_lets_write_a_simple_svelte_preprocessor,
];

export const EXTERNAL_POSTS: ExternalPost[] = [];
