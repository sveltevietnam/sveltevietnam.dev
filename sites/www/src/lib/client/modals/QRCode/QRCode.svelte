...
<div class="relative grid h-full w-full max-w-full place-items-center" bind:this={container}>
  ...
  />
  <div
    class="relative bg-design-bg-1 p-10 border-design-border-1 border"
    use:clickoutside={{ limit: { parent: container } }}
    on:clickoutside={clickOutside}
    transition:fly={{ duration: 200, y: 50 }}
  >
    <button class="absolute top-6 right-6" on:click={dismiss}>
      <svg inline-src="google/close" width="24" height="24" class="fill-current" />
    </button>
    <div class="space-y-6">
      <p class="tp-h2 after:mt-2 after:separator font-medium">{texts.title}</p>
