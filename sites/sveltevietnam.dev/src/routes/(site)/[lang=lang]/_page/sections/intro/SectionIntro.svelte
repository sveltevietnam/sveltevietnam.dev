...
<section
	...
	bind:this={elSection}
>
	<div class="tablet:space-y-20 desktop:space-y-24 space-y-15">
		<h1 class="relative mx-auto w-fit">
			<svg
				class="tablet:h-50 desktop:h-75 relative z-1 mx-auto h-26 w-auto"
				inline-src="./images/title"
				bind:this={elTitle}
			></svg>
			<span class="sr-only"><T key="pages.home.intro.heading" /></span>

			<!-- absolute ornaments -->
			<svg
				class="tablet:-left-18 tablet:-top-24 desktop:-left-24 desktop:-top-28 tablet:h-48
				desktop:h-61 absolute -top-14 -left-11 z-0 h-28 w-auto"
				inline-src="./images/rect-washer"
				bind:this={elRectWasher}
			></svg>
			<svg
				class="tablet:-top-6 tablet:h-34.5 tablet:-right-28 desktop:-right-40 desktop:h-50 absolute -top-3 -right-12 z-0 h-20 w-auto"
	...
	<div class="-z-px absolute inset-0 max-w-full overflow-hidden">
		<img
			class="absolute top-1/2 left-1/2 max-w-none -translate-1/2 opacity-40"
			src={svgMesh}
			alt=""
			width="1929"
			height="1647"
		/>
		<div
			class="tablet:w-150 tablet:h-150 desktop:w-200 desktop:h-200 from-surface absolute top-1/2 left-0 h-100 w-100 -translate-x-1/2
			-translate-y-1/2 rounded-full bg-radial to-transparent"
		>
			<!-- left gradient circle -->
		</div>
		<div
			class="tablet:w-150 tablet:h-150 from-surface absolute top-1/2 right-0 h-100 w-100 translate-x-1/2
			-translate-y-1/2 rounded-full bg-radial to-transparent"
		>
			<!-- left gradient circle -->
		</div>
		<div class="from-surface to-transprent absolute inset-x-0 bottom-0 h-50 bg-gradient-to-t"></div>
	</div>
</section>
