...
Trong [phiên bản thứ 4](https://tailwindcss.com/blog/tailwindcss-v4-beta) (đang trong giai đoạn beta tại thời điểm cập nhật này), Tailwind đã có hiều thay đổi, đặc biệt là đẩy mạnh tích hợp với hệ sinh thái Vite thông qua việc cung cấp Vite plugin chính thức. Tuy nhiên, phần lớn API vẫn không đổi và những đánh giá trong bài viết này vẫn giữ nguyên giá trị.
...
Những giá trị trong thiết kế được gọi chung là "design token" và như bạn thấy ở trên, chúng có thể được khai báo trong CSS (chỉ khả thi từ Tailwind V4), hoặc bằng Javascript - có nghĩa là ta có thể làm bất cứ gì mà Javascript cho phép: map, filter, biến đổi màu sắc, sử dụng API từ Tailwind hay thư viện thứ ba, ... Bằng cách cấu hình trên, ta mặc nhiên có sẵn hai phương thức sử dụng:
...
2. truy cập trong CSS bằng các biến từ Tailwind:
...
```svelte title="Markup - Tailwind tối giản"
:::diff -
<button class="mt-4 px-4 py-2 text-center font-semibold rounded-md bg-white hover:bg-black hover:text-white">
:::
:::diff +
...
Trong phiên bản [version 4](https://tailwindcss.com/blog/tailwindcss-v4-beta), Tailwind không còn
chỉ là một plugin PostCSS nữa mà đã phát triển thành một công cụ xử lý CSS riêng biệt. Package
[postcss-color-scheme](https://github.com/vnphanquang/postcss-color-scheme) đã được cập nhật để
tương thích với Tailwind V4 và hỗ trợ API tương đương như đã giới thiệu ở trên.
