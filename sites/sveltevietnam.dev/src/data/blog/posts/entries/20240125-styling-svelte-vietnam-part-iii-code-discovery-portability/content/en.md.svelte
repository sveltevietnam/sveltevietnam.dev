...
This section is added in January, 2025.
</p>

This blog post was originally written for Tailwind V3. In [Tailwind V4](https://tailwindcss.com/blog/tailwindcss-v4-beta) (in beta as of this writing), the setup procedure for CSS component is simplified and can now be done directly in CSS. I hope, however, that the concepts and techniques discussed here still provides to be a good reference resource.

We can declare a CSS component in Tailwind V4 simply as follows:
...
In V4, Tailwind will organize everything decalred within the `@utilty` rule into the CSS `utilities` layer. To ensure encapsulation, we can further wrap the target CSS inside a `@layer components` block as seen above. Note that, the output CSS will be nested inside `utilities.components`
