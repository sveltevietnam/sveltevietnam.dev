...
```svelte title="Animation & transition"
...
```svelte title="Svelte action - 'externalLink'"
<script>
...
```javascript title="Svelte Preprocessor"
...
```svelte title="Static vs Dynamic"
<!-- tĩnh -->
...
```javascript title="Explicitly external"
// :::diff -
- let external = false;
// :::
// :::diff +
...
Tóm lại, Svelte preprocessor là một đoạn mã trong quy trình hoạt động của Svelte compiler, được dùng để biến đổi mã nguồn Svelte tùy theo nhu cầu. Trong bài viết này, ta đã viết một preprocessor để thêm các thuộc tính `rel` và `target` nếu phát hiện thẻ `a` là đường dẫn ngoài. Thông qua ví dụ đó, ta đã thấy được cách vận hành cơ bản của một preprocesor và sự linh hoạt của nó. Svelte preprocessor có thể dùng để làm nhiều việc hơn nữa, và thật ra là chúng ta có thể không để ý rằng ta đã và đang dùng preprocessor rồi đấy. Hy vọng bài viết sẽ giúp bạn có thêm một công cụ trong hành trình lập trình của mình với Svelte.
