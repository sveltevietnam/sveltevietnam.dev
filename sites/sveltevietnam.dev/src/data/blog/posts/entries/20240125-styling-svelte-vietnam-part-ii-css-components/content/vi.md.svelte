...
Chuỗi bài viết "Styling cho Svelte Việt Nam" được chia làm ba phần, liệt kê sau đây. Bạn đang đọc phần thứ hai.
...
Chú ý ta dùng [:where](https://developer.mozilla.org/en-US/docs/Web/CSS/:where) để kiểm soát [specificity](https://developer.mozilla.org/en-US/docs/Web/CSS/Specificity) của selector này để có thể ghi đè lên tùy vào trường hợp cụ thể. Đây là kỹ thuật phổ biến đặc biệt hay gặp trong các thư viện CSS. Kỹ thuật này có thể được tối giản bằng cách sử dụng [CSS layer](https://developer.mozilla.org/en-US/docs/Web/CSS/@layer) là một tính năng mới của CSS hiện đại.
...
- Nếu thành phần đòi hỏi markup cụ thể hoặc chứa logic đặc biệt, *cân nhắc* tạo Javascript component. Ví dụ cho trường hợp này tại *sveltevietnam.dev* là `Breadcrumbs`, `BlogPostListItem`, ...
