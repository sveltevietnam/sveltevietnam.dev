...
export async function loadPerson(
	...
	let { avatar, popImage, ogImage, ...rest } = person;
	[avatar, popImage, ogImage] = await Promise.all([
		optionalModules === true || optionalModules?.avatar
			? avatar
				? avatar
				: loadPersonAvatar(id)
			: undefined,
		optionalModules === true || optionalModules?.popImage
			? popImage
				? popImage
				: loadPersonPopImage(id)
			: undefined,
		optionalModules === true || optionalModules?.ogImage
			? ogImage
				? ogImage
				: loadPersonOgImage(id, lang)
			: undefined,
	]);
