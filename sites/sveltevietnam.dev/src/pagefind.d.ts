 ...
 * Copied from vite-plugin-pagefind
 */
declare module '@pagefind' {
	...
	/**
	 * Searches the Pagefind index for the given term with a debounce.
	 */
	...
	/**
	 * Returns the filter counts for the current index.
	 */
	function filters(): Promise<PagefindFilterCounts>;

	/**
	 * Initializes the Pagefind instance.
	 */
	 ...
	 */
	function mergeIndex(indexPath: string, options?: Record<string, unknown>): Promise<void>;

	/**
	 * Configures the Pagefind instance with options.
	 */
	function options(options: PagefindIndexOptions): Promise<void>;

	/**
	 * Preloads the results for the given term.
	 */
	function preload(term: string, options?: PagefindIndexOptions): Promise<void>;
